    ...
    <div className="space-y-4">
      ...
      <div className="bg-white rounded-3xl border border-slate-200/80 shadow-xs p-5 space-y-4">
        <p className="text-[10px] font-extrabold text-slate-400 uppercase tracking-wider">
          Available Facilities ({hospitals ? hospitals.length : 0})
        </p>

        {/* Scrollable Hospital Options Container */}
        <div className="space-y-3 max-h-[460px] overflow-y-auto pr-1.5 custom-scrollbar">
          {hospitals && hospitals.map((hosp) => (
            <HospitalCard
              key={hosp.id}
              hospital={hosp}
              isSelected={String(hosp.id) === String(selectedHospitalId)}
        ...
        {/* Bottom Note */}
        <div className="pt-2 flex items-start space-x-2 text-xs text-slate-500 border-t border-slate-100">
